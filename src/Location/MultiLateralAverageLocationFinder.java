...
public class MultiLateralAverageLocationFinder implements LocationFinder {
	...
	public MultiLateralAverageLocationFinder() {
	...
	public Position triangulate(MacRssiPair[] data) {
		...
		double[] distanceToAP = new double[filteredData.length];
//		double sumOfFactors = 0;
		...
		for (double errorMargin = 1; !isInRanges ; errorMargin += 0.05) {
			...
			for (MacRssiPair mac : filteredData) {
				positions[iterationCounter] = knownLocations.get(mac.getMacAsLong());
				distanceToAP[iterationCounter] = getDistanceFromRSSI(bestData.get(mac.getMacAsLong())) * errorMargin;
